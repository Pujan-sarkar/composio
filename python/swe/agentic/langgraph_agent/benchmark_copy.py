...
def run_agent_function(workspace_id: str, issue_config: IssueConfig) -> str:
    ...
    graph, composio_toolset, test_response_file = get_agent_graph(repo_name=issue_config.repo_name.split("/")[-1] , workspace_id=workspace_id, test_command=test_command)
    ...
    if "scikit-learn" in issue_config.repo_name:
        ...
        composio_toolset.execute_action(
            action=Action.SHELLTOOL_EXEC_COMMAND,
            params={"cmd": "python -m pip install -v --no-use-pep517 --no-build-isolation -e ."},
        )
    ...
    if "astropy" in issue_config.repo_name:
        composio_toolset.execute_action(
            action=Action.SHELLTOOL_EXEC_COMMAND,
            params={"cmd": "sed -i 's/requires = [\"setuptools\",/requires = [\"setuptools==68.0.0\",/' pyproject.toml"},
        )
        composio_toolset.execute_action(
            action=Action.SHELLTOOL_EXEC_COMMAND,
    ...
    if "pytest" in issue_config.repo_name:
        ...
        composio_toolset.execute_action(
            action=Action.SHELLTOOL_EXEC_COMMAND,
            params={"cmd": "sed -i \"s/__version__ = version = '.*'/__version__ = version = '8'/\" src/_pytest/_version.py"},
        )
        composio_toolset.execute_action(
            action=Action.SHELLTOOL_EXEC_COMMAND,
            params={"cmd": "sed -i \"s/__version_tuple__ = version_tuple = .*/__version_tuple__ = version_tuple = (8)/\" src/_pytest/_version.py"},
        )
    ...
    try:
        run_result = graph.invoke(
            ...
            {"recursion_limit": 70, "timeout": 1200},  # Added timeout of 20 minutes (1200 seconds)
        )
